  ...
  de: {
    ...
    website: {
      ...
      faq: { kicker: 'Gut zu wissen', title: 'Vor Ihrer', emphasis: 'neuen Website.', items: [
        { question: 'Was kostet eine neue Website?', answer: 'Der Preis richtet sich nach Seitenzahl, Inhalten und gewünschten Funktionen. Wir besprechen den Umfang und erstellen Ihnen ein konkretes Angebot. Eine laufende Betreuung stimmen wir gesondert mit Ihnen ab.' },
  ...
  en: {
    ...
    website: {
      ...
      faq: { kicker: 'Good to know', title: 'Before your', emphasis: 'new website.', items: [
        { question: 'How much does a new website cost?', answer: 'The price depends on the number of pages, content and required features. We discuss the scope and provide a specific proposal. Ongoing support is agreed separately.' },
...
export const serviceMenu = (locale: Locale) => {
  ...
  const itemLink = (index: number) => ({ label: items[index].title, description: items[index].details[0], href: index === 0 ? paths.website : `${paths.overview}#${items[index].id}` });
  return [
